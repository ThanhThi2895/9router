...
The `Dockerfile` defaults to the official mirrors (`dl-cdn.alpinelinux.org`,
`registry.npmjs.org`). The deploy workflow passes the same values explicitly
through the `ALPINE_MIRROR` / `NPM_REGISTRY` build args. Override them the same
way for a build that needs a regional mirror:
